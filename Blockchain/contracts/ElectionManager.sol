...
contract ElectionManager is AccessControl {
    ...
    bytes32 public constant ELECTION_AUTHORITY =
        keccak256("ELECTION_AUTHORITY");
    ...
    modifier onlyManagerOrSuperAdmin() {
        require(
            hasRole(ELECTION_MANAGER, msg.sender) ||
                isFactorySuperAdmin(msg.sender),
            "Not authorized"
        );
        _;
    }
    ...
    // Check factory SUPER_ADMIN role
    ...
    ) public onlyManagerOrSuperAdmin returns (bool) {
    ...
    function startRegistration(
        ...
        require(
            electioninfo[_id].currentPhase == ElectionPhase.Created,
            "Wrong phase"
        );
